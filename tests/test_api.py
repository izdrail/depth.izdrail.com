...
def test_health_reports_model_asset_readiness(tmp_path):
    app.state.inference = type("Stub", (), {"loaded": False, "assets_dir": tmp_path})()
...
def test_prediction_contract_and_original_size():
    stub = type("Stub", (), {})()
    stub.predict_async = AsyncMock(
        return_value=Prediction(
            np.ones((256, 256), dtype=np.float32),
            np.ones((8, 12, 3), dtype=np.uint8),
            12.5,
        )
    )
    ...
    raw = np.load(
        io.BytesIO(base64.b64decode(body["prediction_npy_base64"])), allow_pickle=False
    )
...
def test_rejects_bad_resolution_and_checkpoint():
    ...
    assert (
        client.post(
            "/api/predict", files=files, data={"resolution": "1024"}
        ).status_code
        == 422
    )
    assert (
...
def test_rejects_malformed_and_wrong_type():
    ...
    assert (
        client.post(
            "/api/predict", files={"image": ("x.png", b"bad", "image/png")}
        ).status_code
        == 400
    )
    assert (
        client.post(
            "/api/predict", files={"image": ("x.gif", b"bad", "image/gif")}
        ).status_code
        == 415
    )
